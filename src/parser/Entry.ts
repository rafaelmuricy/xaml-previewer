...
export function renderEntry(node: XmlNode, ctx: RenderContext): string {
	...
	return `<div data-xaml="Entry"${styleAttr(host)}${props.attrs}>${headerHtml(getAttr(node, 'Header'))}<textarea spellcheck="false" rows="1"${placeholderAttr}${styleAttr(fill)}>${escapeHtmlText(text)}</textarea></div>`;
}
